...
class Test extends Command
{
    ...
    public function handle()
    {
        ...
        DB::table('credentials')->where('id', 1)->update([
            'username' => encrypt('123456'),
            'password' => encrypt('123456'),
        ]);

        dispatch_sync(new \App\Jobs\GetReadyToSendSms());
        ...
        ])->post("https://sentry.io/api/0/organizations/{$organizationSlug}/members/", [
            'email' => $userEmail,
            'role' => 'member',
            'sendInvite' => false
        ]);
...
// Handle response
        if ($response->successful()) {
            ...
            return response()->json(['success' => true, 'data' => $memberData]);
        } else {
            // Handle error
            $error = $response->json();
            \Log::error('Sentry API Error: ' . json_encode($error));
            return response()->json(['success' => false, 'error' => $error], 400);
        }

        return;
        $token = '2196f2f66f95d2c0765b793613c88fa4b9d1c5feae26df98b446bdc07d271792';

        $gm = new Api($token);

        $chat_id = '+989157009100';
        ...
        // chat_id=%2B989157009100&data=Hello+world&type=text


        try {

            $client = new Client();
            $response = $client->post($this->baseURL . $method, [
                'headers' => [
                    'token' => $this->token,
                ],
                'form_params' => $params,
            ...
            $curl_result = $response->getBody()->getContents();
            $httpcode = $response->getStatusCode();


            $response = Http::withHeader('token', '2196f2f66f95d2c0765b793613c88fa4b9d1c5feae26df98b446bdc07d271792')
